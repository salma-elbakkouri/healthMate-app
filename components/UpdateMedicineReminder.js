...
import { fetchMedicineReminderById, updateMedicineReminder } from '../config/authFunctions';

const UpdateMedicineReminder = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { reminderId } = route.params; // Passed from the navigation
  ...
  useEffect(() => {
    const loadReminderDetails = async () => {
      try {
        const reminder = await fetchMedicineReminderById(reminderId);
        if (reminder) {
          setMedication(reminder.pillName || ''); // Fallback to empty string if undefined
          setPillCount(reminder.amount || 0);
          setFrequency(reminder.frequency || ''); // Fallback to empty string if undefined
          setBeginDate(new Date(reminder.beginDate) || new Date()); // Default to current date if undefined
          setEndDate(new Date(reminder.endDate) || new Date()); // Default to current date if undefined
          setDoses(reminder.doses || { firstDose: '', secondDose: '', thirdDose: '' });
          setNotificationEnabled(reminder.notificationEnabled || false);
        }
      } catch (error) {
        console.error('Error fetching reminder details:', error);
        Alert.alert('Error', 'Failed to load reminder details. Please try again.');
      }
    };
